...
| --- | --- | --- | --- | --- |
| `data/dictionary/tr/` | Turkish dictionary entry JSON files and `SOURCE.md` | `../dictionary`, commit `bf756f5d432bd84cf3ff7581c6d46eebb0e62473` | 1,680 evidence-enriched entry files | Staged, not in `RELEASE.json`; release metadata/checksums pending. |
...
| `data/bridges/qac-furuq-v4-root-map.sqlite.gz` and `data/bridges/qac-furuq-v4-root-map.tsv` | Root-level QAC-to-furuq_v4 gateway, with bidirectional lookup views | `../latent_activation`, commit `f47613506937b980f2708aed73eca9ef776deb65` | 1,642 QAC roots; 1,654 target rows; 1,455 unique, 92 split, 95 no frozen rooted surface match | Staged, not in `RELEASE.json`; this is the required root identity gateway. Existing `data/bridges/qac-v4.sqlite.gz` remains form-level only. |
